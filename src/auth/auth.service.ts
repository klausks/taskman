...
import { ExpirationJwtCustom, TokenPayload } from './interfaces/auth.interfaces';
...
console.log('user', await this.verifyPassword(plainTextPassword, user?.password))
    try {
      await this.verifyPassword(plainTextPassword, user?.password);
      delete user.password;
      return user;
    } catch (e) {
      throw new HttpException('email ou senha estão incorretos', HttpStatus.BAD_REQUEST);
    }
  }
  ...
  public getJwtToken(userId: number, expirationJwtCustom?: ExpirationJwtCustom) {
